class Employee < ActiveRecord::Base
  ...
  Devise.setup do |config|
    ...
    config.saml_configure do |settings|
      settings.assertion_consumer_service_url     = "http://localhost:8080/employees/saml/auth"
      settings.assertion_consumer_service_binding = "urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST"
      settings.name_identifier_format             = "urn:oasis:names:tc:SAML:2.0:nameid-format:transient"
      settings.issuer                             = "https://app.onelogin.com/saml/metadata/#{ENV['ONE_LOGIN_APP_ID_EMPLOYEE']}"
      settings.authn_context                      = "urn:oasis:names:tc:SAML:2.0:ac:classes:PasswordProtectedTransport"
      settings.idp_sso_target_url                 = "https://marmar-is.onelogin.com/trust/saml2/http-post/sso/#{ENV['ONE_LOGIN_APP_ID_EMPLOYEE']}"
      settings.idp_slo_target_url                 = "https://marmar-is.onelogin.com/trust/saml2/http-redirect/slo/#{ENV['ONE_LOGIN_APP_ID_EMPLOYEE']}"
